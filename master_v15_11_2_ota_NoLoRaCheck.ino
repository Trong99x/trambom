...
UniversalTelegramBot bot(BOT_TOKEN_DEFAULT, telegramClient);
...
void performOTAUpdate() {
    ...
    // --- BƯỚC 1: Đọc file version.txt để kiểm tra phiên bản ---
    // Khai báo client cục bộ (local) để tự giải phóng RAM TLS ngay sau khi xong
    WiFiClientSecure verClient;
    verClient.setInsecure();
    verClient.setTimeout(5000); 
    ...
    if (code != HTTP_CODE_OK) {
        sendTelegramDirect(alertBlock(ICON_BAD, "OTA thất bại", "Lỗi HTTP " + String(code)));
        http.end();
        return;
    }
    ...
    remoteVersion.trim();
    http.end(); // Kết thúc session kết nối để giải phóng vùng nhớ

    if (remoteVersion == FW_VERSION) {
        sendTelegramDirect(alertBlock(ICON_OK, "Đã là bản mới nhất", "Bản hiện tại: " + String(FW_VERSION)));
        return;
    }

    // --- BƯỚC 2: Có bản mới -> Kích hoạt tiến trình cập nhật ---
    otaRunning = true; // Chặn luồng check tin nhắn Telegram
    
    // Gửi thông báo chuẩn bị cập nhật
...
bool testTelegramToken(const String &token, const String &chatId) {
    ...
    telegramClient.setTimeout(5000); // Thay thế trực tiếp bằng 5000 (5 giây) chống block luôn tại đây
...
void setupWiFiSingle() {
    Serial.println("\n[WiFi] Khởi tạo kết nối...");
    WiFi.mode(WIFI_STA);
    
    // Sử dụng thông tin cấu hình mặc định ban đầu
    WiFi.begin(WIFI_SSID_DEFAULT, WIFI_PASS_DEFAULT);

    unsigned long startAttempt = millis();
    // Chờ kết nối WiFi trong tối đa 15 giây để tránh treo hàm setup
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // --- ĐOẠN ĐÃ SỬA: Cấu hình an toàn cho telegramClient chống block WDT ---
        telegramClient.setInsecure();
        telegramClient.setTimeout(5000);          // Thời gian chờ phản hồi tối đa 5 giây
        telegramClient.setHandshakeTimeout(5);    // Thời gian bắt tay TLS tối đa 5 giây
    } else {
        Serial.println("\n[WiFi] Kết nối THẤT BẠI (Hết thời gian chờ). Sẽ thử lại sau trong các Task.");
    }
}
...
void initWatchdog() {
    // Sửa cấu hình timeout thành 30 giây để tránh reset khi mạng chập chờn
...
void vTelegramTask(void *pvParameters) {
    ...
    for (;;) {
        esp_task_wdt_reset();
        // --- SỬA TẠI ĐÂY: Nếu đang OTA thì bỏ qua việc kiểm tra tin nhắn ---
        if (otaRunning) {
            for (int i = 0; i < 20; i++) {
                esp_task_wdt_reset();
...
// --- SỬA TẠI ĐÂY: Cấu hình timeout chống block cho Telegram ---
